...
# ══════════════════════════════════════════════════
#  P3.6: ETF 对比模式 — 双ETF归一化叠加
# ══════════════════════════════════════════════════
@router.get("/api/sector-etf/compare")
async def api_sector_etf_compare(code1: str, code2: str):
    """双ETF对比：归一化到 base=100，返回叠加数据。"""
    all_etfs = {**INDEX_ETF, **SECTOR_ETF}
    if code1 not in all_etfs and code2 not in all_etfs:
        return {"error": "未知的ETF代码", "series1": [], "series2": [], "name1": "", "name2": ""}

    # 确保数据库已初始化
    get_conn()

    def _get_normalized(code):
        table = "index_etf_daily" if code in INDEX_ETF else "sector_etf_daily"
        df = query(
            f"SELECT trade_date, close FROM {table} WHERE ts_code=:p0 ORDER BY trade_date",
            {"p0": code},
        )
        df = _apply_etf_adj(df, code)
        if df is None or len(df) == 0:
            return [], all_etfs.get(code, code)
        ...
        for _, row in df.iterrows():
            ...
            if len(td) == 8:
                ...
                td_fmt = td
            val = round(float(row["close"]) / base_close * 100, 2)
            normalized.append([td_fmt, val])

        return normalized, all_etfs.get(code, code)

    series1, name1 = _get_normalized(code1)
    series2, name2 = _get_normalized(code2)
...
# ══════════════════════════════════════════════════
#  P1.3: Pre-compute ETF anomalies → etf_anomalies table
# ══════════════════════════════════════════════════


def precompute_all_etf_anomalies():
    """Compute anomalies for all index + sector ETFs and store in etf_anomalies.
    Called after ETF data updates to avoid recomputing on every API request."""
    conn = get_conn()
    try:
        conn.execute(text("DELETE FROM etf_anomalies"))
        conn.commit()
        ...
        for code in all_etfs:
            try:
                table_name = "index_etf_daily" if code in INDEX_ETF else "sector_etf_daily"
                df = query(
                    f"SELECT trade_date, pct_chg FROM {table_name} "
                    "WHERE ts_code=:p0 ORDER BY trade_date",
                    {"p0": code}
                )
                df_share = query(
                    "SELECT trade_date, fd_share FROM etf_share "
                    "WHERE ts_code=:p0 AND trade_date <= :p1 ORDER BY trade_date",
                    {"p0": code, "p1": today_str}
                )

                anomalies = _detect_anomalies(df, df_share)
                rows_to_insert = []

                for a in anomalies.get("price", []):
                    rows_to_insert.append({
                        "ts_code": code,
                        "trade_date": str(a.get("trade_date", "")),
                        "anomaly_type": "price",
                        "z_score": None,
                        "value": float(a.get("pct_chg", 0) or 0),
                    })

                for a in anomalies.get("share", []):
                    rows_to_insert.append({
                        "ts_code": code,
                        "trade_date": str(a.get("trade_date", "")),
                        "anomaly_type": "share",
                        "z_score": float(a.get("z_score", 0) or 0),
                        "value": float(a.get("fd_share", 0) or 0),
                    })

                if rows_to_insert:
                    df_ins = pd.DataFrame(rows_to_insert)
                    df_ins = df_ins.drop_duplicates(subset=["ts_code", "trade_date", "anomaly_type"])
                    get_db_manager().upsert_dataframe(
                        df_ins, "etf_anomalies", ["ts_code", "trade_date", "anomaly_type"]
                    )
            except Exception:
                continue  # Skip individual ETF failures

        logger.info("ETF anomalies pre-computed and stored")
    finally:
        conn.close()
