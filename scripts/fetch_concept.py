    ...
    try:
        ...
        for idx, row in concept_df.iterrows():
            ...
            try:
                ...
                print(f"  [{idx+1}/{len(concept_df)}] {concept_name}: {len(detail_df) if detail_df is not None else 0} 只股票", end='\r')
            except Exception as e:
                print(f"\n[WARN] 获取概念 {concept_name} 失败: {e}")
                continue
        
        print(f"\n[OK] 总共获取到 {len(all_relations)} 条股票-概念关系")
