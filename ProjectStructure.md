...
| `recommendation_engine.py` | **投资建议引擎** | 六因子评分 + RSRS 象限覆盖 + 相关性惩罚 + 大盘择时 + 仓位配置 |
...
| 路由文件 | 主要端点 |
|----------|----------|
| `overview.py` | `GET /`（首页）、`GET /api/overview`、`GET /api/heatmap`、`GET /health` |
| `analysis.py` | `GET /analysis`、`GET /api/analysis/presets`、因子分布/IC/象限/ICIR 图表 API、投资建议 API |
| `etf.py` | `GET /etf`、`GET /sector`、指数/行业 ETF 详情 API |
| `fetch.py` | `GET /fetch`、`POST /api/fetch`、数据状态/缓存管理 API |
| `telemetry.py` | 遥测端点 |
...
| 文件 | 职责 |
|------|------|
| `cache.py` | 双缓存策略：内存 LRU（始终开启，maxsize=1000）+ Redis（可选） |
| `middleware.py` | 令牌桶限流（60 req/min）；Cache-Control 头 |
...
| 008_convert_trade_date_to_date | trade_date 类型转换 |

---

## ETF 覆盖范围

- **指数 ETF（5只）**：沪深300、中证500、上证50、中证1000、科创50
- **行业 ETF（32只）**：半导体、新能源车、医药、银行、证券、消费、通信、军工、光伏、黄金、农业、旅游、软件、金融科技、医疗、游戏、电池、稀土、高端装备、能源、科技、电网设备等
...
`scripts/` 目录包含 25+ 个独立 Python 脚本和 Shell 脚本，用途包括：
